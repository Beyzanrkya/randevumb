...
export default function BusinessRegisterScreen() {
  ...
  const handleRegister = async () => {
    ...
    setLoading(true);
    try {
      console.log("İşletme kayıt isteği gönderiliyor:", { businessName, email });
      ...
      console.log("İşletme kayıt yanıtı alındı:", response.data);
      if (response.data) {
        setOtpModalVisible(true);
        console.log("İşletme OTP Modalı açılmalı: true");
      }
    } catch (error: any) {
      console.error("İşletme kayıt hatası:", error.response?.data || error.message);
  ...
  const handleVerifyOTP = async () => {
    ...
    setVerifying(true);
    try {
      const response = await api.post('/businesses/verify-email', { email, code: otpCode });
      if (response.data) {
        Alert.alert('Başarılı', 'İşletme e-posta adresiniz doğrulandı! Şimdi giriş yapabilirsiniz.');
      ...
      <Modal visible={otpModalVisible} animationType="fade" transparent={true}>
        <View style={styles.otpModalOverlay}>
          <View style={[styles.otpCard, { backgroundColor: theme.card }]}>
            ...
            <Text style={[styles.otpTitle, { color: theme.text }]}>İşletmenizi Doğrulayın</Text>
