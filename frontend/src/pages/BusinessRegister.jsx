...
export default function BusinessRegister() {
  const [form, setForm] = useState({ name: "", email: "", password: "", categoryId: "" });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setMessage(res.data.message || "Kayıt başarılı! Doğrulama sayfasına yönlendiriliyorsunuz...");
      setIsError(false);
  ...
  return (
    <div className="auth-container">
      ...
      <div className="auth-left">
        <img src="/ai_business_register.png" alt="MBrandev Salon" style={{ width: "100%", height: "100%", objectFit: "cover", position: "absolute", top: 0, left: 0, zIndex: 0 }} />
        <div style={{ position: "absolute", top: 0, left: 0, right: 0, bottom: 0, background: "linear-gradient(135deg, rgba(142, 74, 93, 0.4), rgba(30, 42, 64, 0.6))", zIndex: 1 }}></div>
        ...
        <div style={{ position: "relative", zIndex: 2, color: "#1E2A40", background: "rgba(255, 255, 255, 0.85)", backdropFilter: "blur(12px)", padding: "40px", borderRadius: "32px", boxShadow: "0 20px 40px rgba(0,0,0,0.1)", display: "inline-block", maxWidth: "500px" }}>
          <h2 style={{ fontSize: "48px", fontWeight: "800", marginBottom: "16px", lineHeight: "1.1", letterSpacing: "-1px", color: "#1E2A40" }}>Aramıza<br/>Katılın</h2>
          <p style={{ fontSize: "18px", color: "#4b5563", lineHeight: "1.6", fontWeight: "500", margin: 0 }}>MBrandev ailesine katılarak işletmenizi binlerce potansiyel müşteriye ulaştırın ve rezervasyonlarınızı dijitalleştirin.</p>
        </div>
      </div>
      ...
      <div className="auth-right">
        ...
        <div style={{ width: "100%", maxWidth: "450px", margin: "0 auto" }}>
          <div style={{ marginBottom: "40px" }}>
            <div style={{ display: "inline-block", padding: "6px 14px", background: "rgba(30, 42, 64, 0.1)", color: "#8E4A5D", borderRadius: "20px", fontWeight: "700", fontSize: "12px", marginBottom: "16px", letterSpacing: "1px", textTransform: "uppercase" }}>Kurumsal Başvuru</div>
            <h2 style={{ fontSize: "36px", fontWeight: "800", color: "#1E2A40", marginBottom: "10px", letterSpacing: "-0.5px" }}>İşletme Hesabı Açın</h2>
            <p style={{ color: "#6b7280", fontSize: "16px", lineHeight: "1.5" }}>Hemen ücretsiz işletme hesabınızı oluşturun.</p>
          </div>

          <form onSubmit={handleSubmit}>
            <div style={{ marginBottom: "20px" }}>
              <label style={{ display: "block", fontSize: "14px", fontWeight: "600", color: "#374151", marginBottom: "8px" }}>İşletme Adı</label>
              <input
                type="text"
            ...
            <div style={{ marginBottom: "20px" }}>
              <label style={{ display: "block", fontSize: "14px", fontWeight: "600", color: "#374151", marginBottom: "8px" }}>İşletme Kategorisi</label>
              <select
                value={form.categoryId}
                onChange={(e) => setForm({ ...form, categoryId: e.target.value })}
                style={{ width: "100%", padding: "16px 18px", borderRadius: "14px", border: "1px solid #e5e7eb", fontSize: "15px", outline: "none", transition: "all 0.2s", boxSizing: "border-box", background: "#f9fafb", cursor: "pointer" }}
                onFocus={(e) => { e.target.style.borderColor = "#1E2A40"; e.target.style.background = "#fff"; e.target.style.boxShadow = "0 0 0 4px rgba(30, 42, 64, 0.1)"; }}
                onBlur={(e) => { e.target.style.borderColor = "#e5e7eb"; e.target.style.background = "#f9fafb"; e.target.style.boxShadow = "none"; }}
                required
              >
          ...
          <div style={{ marginTop: "40px", textAlign: "center", fontSize: "15px", color: "#6b7280", borderTop: "1px solid #f3f4f6", paddingTop: "30px" }}>
            Zaten bir işletme hesabınız var mı? <br/>
            <Link to="/login" style={{ color: "#8E4A5D", fontWeight: "700", textDecoration: "none", display: "inline-block", marginTop: "8px", transition: "opacity 0.2s" }} onMouseOver={(e) => e.target.style.opacity = "0.8"} onMouseOut={(e) => e.target.style.opacity = "1"}>Giriş Yapın</Link>
          </div>
